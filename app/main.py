...
from app.access_control import AccessControlResult, detect_access_control, skipped_creator_from_access
...
def access_control_response(
    ...
    return AnalyzeProfileResponse(
        ...
        llm_provider=get_analyzer_metadata()["llm_provider"] if debug else None,
        llm_model=get_analyzer_metadata()["llm_model"] if debug else None,
        api_key_detected=get_analyzer_metadata()["api_key_detected"] if debug else None,
    )


def manual_verification_unavailable_response(platform_name: str, message: str = MANUAL_VERIFICATION_UNAVAILABLE_MESSAGE) -> JSONResponse:
    return JSONResponse(
        {
